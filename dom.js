$(document).ready(function(){
	...
	// - html() : 태그사용 가능. 모양을 변경(text보다 큰 그룹 개념)
	...
	// $('.list1 li:eq(3)').after('<li><a href="#">BOARD</a></li>') > 순서 잘 확인하기
	$('.list1 li:eq(4)').after('<li><a href="#">BOARD</a></li>')
	
	// [태그 이동]
	// $('선택자').명령어('이동할 위치')
	// prependTo() / appendTo() **To 첫글자 대문자 : 선택자를 타겟의 안쪽으로 맨앞/맨뒤로 이동 
	...
	// $('.list1').eq(2).insertBefore('.list2 li:eq(2)') 선택자만 .eq()이렇게 가능 이동위치는 반드시 :eq()로 써야함. ~까지로 인식해버림
	$('.list1 li').eq(2).insertBefore('.list2 li:eq(2)')
	
	// 10. list1의 'BOARE'를 list2의 menu3뒤로 이동
	...
	// 12. list2의 menu4의 안에 있는 내용을 비워라/지워라
	$('.list2 li').eq(5).empty()
	
	// [wrap - 감싸는 태그 추가]
	// .wrap() / .wrapAll() / .wrapInner()
	// .wrap() : 타겟이 여러개일 경우 따로따로 해당 태그로 감싸진다.
	// 13. list1을 <nav class="nav_wrap">으로 감싸도록 설정
	...
	// 15. body 안에 잇는 내용을 <div id="wrap"></div>으로 감싸도록 설정
